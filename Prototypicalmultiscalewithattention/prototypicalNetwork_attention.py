...
class Self_Attn(nn.Module):
    ...
    def __init__(self,in_dim,activation,gamma,gammalearnable,gamma1):
        ...
        self.gamma = gamma
        self.gammalearnable=gammalearnable
        self.gamma1=gamma1
...
class PrototypicalNetworks(nn.Module):
    def __init__(self,backbone:nn.Module,weightlearnable,gammalearnable,weight,gamma1,gamma2):
        ...
        self.gamma1_l  =   nn.Parameter(torch.Tensor([gamma1]))
        self.gamma2_l  =   nn.Parameter(torch.Tensor([gamma2]))
    ...
    def calculate(self,support_images:torch.Tensor,support_labels:torch.Tensor,query_images: torch.Tensor,num,numker):
        ...
        self_attention = Self_Attn(in_dim=n_channels,activation='relu',gamma=self.gamma1_l,gammalearnable=self.gammalearnable,gamma1=self.gamma1).to('cuda')
        ...
        n_channels = input_size.size()[1]
        self_attention = Self_Attn(in_dim=n_channels,activation='relu',gamma=self.gamma2_l,gammalearnable=self.gammalearnable,gamma1=self.gamma1).to('cuda')
        output,_ = self_attention(input_size)
        # print(output[0][0])
